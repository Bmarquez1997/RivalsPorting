...
#include "Processing/RivalsPortingTexturePipeline.h"
...
#include "Utilities/RivalsPortingUtils.h"
...
void FImportContext::ImportTextureData(const FJsonWrapper& ExportData)
{
	...
	for (const auto& TextureJson : Textures)
	{
		...
		auto* Pipeline = NewObject<URivalsPortingTexturePipeline>(GetTransientPackage());
...
UMaterialInstanceConstant* FImportContext::ImportMaterial(const FJsonWrapper& MaterialData)
{
	...
	if (FRivalsPortingUtils::Any<FJsonWrapper>(MaterialData.GetArray("Switches"), [&](const FJsonWrapper& Item)
	{
		return FNames::LayerSwitchNames.Contains(Item.Get<FString>("Name"));
	}) && FRivalsPortingUtils::Any<FJsonWrapper>(MaterialData.GetArray("Textures"), [&](const FJsonWrapper& Item)
	{
	...
	if (FRivalsPortingUtils::Any<FString>(FNames::HeroBaseMaterialNames, [&](const FString& Name)
	{
	...
	if (FRivalsPortingUtils::Any<FString>(FNames::EyeBaseMaterialNames, [&](const FString& Name)
	{
		return BaseMaterialPath.Contains(Name, ESearchCase::IgnoreCase);
	}))
	{
	...
	if (FRivalsPortingUtils::Any<FString>(FNames::EyeGlassBaseMaterialNames, [&](const FString& Name)
	{
		return BaseMaterialPath.Contains(Name, ESearchCase::IgnoreCase);
	}))
	{
	...
	if (FRivalsPortingUtils::Any<FString>(FNames::TranslucentBaseMaterialNames, [&](const FString& Name)
	{
		return BaseMaterialPath.Contains(Name, ESearchCase::IgnoreCase);
	}))
	{
