...
{
	Thread = FRunnableThread::Create(this, TEXT("RivalsPortingListenServer"));
}
...
{
	UE_LOG(LogRivalsPorting, Log, TEXT("Running FP V4 Server at %s"), *Endpoint.ToString())
	...
	while (bIsRunning)
	{
		...
		{
			UE_LOG(LogRivalsPorting, Warning, TEXT("Failed to accept client connection"));
			continue;
		}
        
		UE_LOG(LogRivalsPorting, Log, TEXT("Client connected from %s"), *RemoteAddress->ToString(true));
...
void FListenServer::Stop()
{
	...
	UE_LOG(LogRivalsPorting, Log, TEXT("Shutdown FP V4 Server"))
}
...
void FListenServer::HandleClient(FSocket* ClientSocket)
{
	...
	while (bIsRunning)
	{
		...
		auto ReceivedString = FRivalsPortingUtils::BytesToString(Data);

		switch (Header->Type)
		{
		case EMessageCommandType::Message:
			UE_LOG(LogRivalsPorting, Log, TEXT("%s"), *ReceivedString)
			break;
	...
	UE_LOG(LogRivalsPorting, Log, TEXT("Client disconnected: %s"), *ClientAddress->ToString(true));
