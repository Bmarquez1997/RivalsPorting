...
using RivalsPorting.Models.Levels;
...
public partial class SupabaseService : ObservableObject, IService
{
    ...
    public SupabaseService(APIService api)
    {
        ...
        {
            var auth = await Api.RivalsPorting.AuthInfo();
            if (auth is null)
            {
    ...
    {
        await Api.RivalsPorting.PostExports(objectPaths);
    }
    ...
    private async Task PostLogin()
    {
        ...
        await Api.RivalsPorting.PostLogin();
        _postedLogin = true;
    }
    ...
    public async Task<UserInfoResponse?> GetUserAsync(string? id)
    {
        ...
        var userInfo = await Api.RivalsPorting.UserInfo(id);
