...
public partial class HelpViewModel : ViewModelBase
{
    ...
    {
        var response = await Api.RivalsPorting.GetArticles();
            ...
            new DialogButton
            {
                ...
                Action = async () =>
                {
                    ...
                    for (var i = 0; i < newArticle.Sections.Count; i++)
                    {
                        ...
                        var uploaded = await Api.RivalsPorting.UploadArticleImage(bytes, fileName);
                    ...
                    if (!string.IsNullOrEmpty(newArticle.Id))
                        await Api.RivalsPorting.UpdateArticle(newArticle.Id, request);
                    else
                        await Api.RivalsPorting.CreateArticle(request);
        ...
        Info.Dialog($"Are you sure you would like to delete \"{article.Title}\"?",
            ...
            [
                new DialogButton
                {
                    ...
                    Action = async () =>
                    {
                        await Api.RivalsPorting.DeleteArticle(article.Id);
                        await UpdateArticles();
                    }
                }
            ]);
